    ...
    async with gagan.conversation(event.chat_id) as conv: 
        try:
            ...
            cd = await conv.send_message("**Batch process ongoing...**\n\nProcess completed: ", 
                                    buttons=[[Button.url("𝐉𝐨𝐢𝐧 𝐂𝐡𝐚𝐧𝐧𝐞𝐥", url="http://t.me/WarriorUnitsBots")]])
    ...
    for i in range(len(ids_data[str(sender)])):
        ...
        try: 
            ...
            await countdown.edit(count_down, 
                                 buttons=[[Button.url("𝐉𝐨𝐢𝐧 𝐂𝐡𝐚𝐧𝐧𝐞𝐥", url="https://t.me/WarriorUnits")]])
            ...
            if int(fw.value) > 300:
                ...
                try:
                    ...
                    if countdown.text != count_down:
                        await countdown.edit(count_down, buttons=[[Button.url("𝐉𝐨𝐢𝐧 𝐂𝐡𝐚𝐧𝐧𝐞𝐥", url="http://t.me/WarriorUnitsBots")]])
        except Exception as e:
            #logger.info(e)
            #await client.send_message(sender, f"An error occurred during cloning, batch will continue.\n\n**Error:** {str(e)}")
            if countdown.text != count_down:
                await countdown.edit(count_down, buttons=[[Button.url("𝐉𝐨𝐢𝐧 𝐂𝐡𝐚𝐧𝐧𝐞𝐥", url="https://t.me/WarriorUnitsBots")]])
        n = i + 1
